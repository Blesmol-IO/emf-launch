...
public class TestVogellaThread extends TestDebugElement implements IThread {
	...
	public TestVogellaThread(IDebugTarget target, String wordToBeSpelled) {
		super(target);
	...
	@Override
	public TestVogellaStackFrame getTopStackFrame() throws DebugException {
		synchronized (this) {
			if (isSuspended()) {
				return new TestVogellaStackFrame(this, wordToBeSpelled, charPos);
			}
		}
		return null;
	}
	...
	@Override
	public TestVogellaStackFrame[] getStackFrames() throws DebugException {
		synchronized (this) {
			if (isSuspended()) {
				if (charPos < 1) {
					return new TestVogellaStackFrame[] { new TestVogellaStackFrame(this, wordToBeSpelled, charPos) };
				}
				TestVogellaStackFrame[] frames = new TestVogellaStackFrame[charPos + 1];
				for (int i = charPos; i >= 0; i--) {
					frames[i] = new TestVogellaStackFrame(this, wordToBeSpelled, charPos - i);
				}
				return frames;
			}
		}
		return new TestVogellaStackFrame[0];
	}
	...
	class StepOver implements Runnable {
		@Override
		public void run() {
			synchronized (TestVogellaThread.this) {
			...
			case TERMINATED:
				fireTerminateEvent();
				TestVogellaDebugTarget target = (TestVogellaDebugTarget) getDebugTarget();
				target.dispose();
				break;
			default:
				break;
